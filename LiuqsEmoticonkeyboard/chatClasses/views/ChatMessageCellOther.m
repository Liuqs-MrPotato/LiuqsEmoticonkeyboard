...
@interface ChatMessageCellOther ()
...
@implementation ChatMessageCellOther
...
+ (instancetype)cellWithTableView:(UITableView *)tableview {
    ...
    NSString *ID = @"ChatMessageCellOther";
    _tableview = tableview;
    ChatMessageCellOther *cell = [tableview dequeueReusableCellWithIdentifier:ID];
    if (!cell) {
        cell = [[ChatMessageCellOther alloc]initWithStyle:UITableViewCellStyleDefault reuseIdentifier:ID];
    }
    return cell;
}
...
- (void)LayoutSubviews {
    ...
    [self.airView mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.equalTo(self.messageContentLabel.mas_left).offset(-18);
        make.right.equalTo(self.messageContentLabel.mas_right).offset(9);
...
- (void)setMessage:(ChatMessage *)message {
    ...
    UIImage *norImage  = [UIImage resizebleImageWithName:@"chat_receive_nor"];
